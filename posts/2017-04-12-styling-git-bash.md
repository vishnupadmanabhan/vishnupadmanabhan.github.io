...
So how do we do it? To make this change we need to go into the `C:\Program Files\Gittc\profile.d\` folder and find the `git-prompt.sh` file. It looks something like this: 
``` Bash
    ...
    # set window title
    PS1='\[]0;$TITLEPREFIX:${PWD//[^[:ascii:]]/?}\]' 
    PS1="$PS1"'
'                 # new line
    PS1="$PS1"'\[[32m\]'       # change to green
    PS1="$PS1"'\[email protected]\h '             # [email protected]<space>
    PS1="$PS1"'\[[35m\]'       # change to purple 
    PS1="$PS1"'$MSYSTEM '          # show MSYSTEM
    PS1="$PS1"'\[[33m\]'       # change to brownish yellow
    ...
    then
        ...
        then
            ...
            PS1="$PS1"'\[[36m\]'  # change color to cyan
            PS1="$PS1"'`__git_ps1` '   # bash function
        fi
    fi
    PS1="$PS1"'\[[0m\]'        # change color
    PS1="$PS1"'
'                 # new line
...
Let us start by modifying the title. I like to keep something like `Git-Bash` with the current working directory in the title. For this, remove `$TITLEPREFIX` from the line `PS1='\[]0;$TITLEPREFIX:${PWD//[^[:ascii:]]/?}\]'` and add something like this `PS1='\[]0;Git-Bash: ${PWD//[^[:ascii:]]/?}\]'`. This will change the title to “Git-Bash: path to current working directory”.
...
Things like `PS1="$PS1"'\[[32m\]'` are used to set the colour of the string that follows. `[32m\]` denotes that the colour has to be green. Here are few colours that can be applied:
